...
public:
    ...
    bool deskewInfo()
    {
        ...
        // 디버깅 출력 추가
        // RCLCPP_INFO(get_logger(), "IMU queue size: %zu", imuQueue.size());
        // if (!imuQueue.empty()) {
        //     RCLCPP_INFO(get_logger(), "IMU time range: %f to %f", 
        //             stamp2Sec(imuQueue.front().header.stamp),
        //             stamp2Sec(imuQueue.back().header.stamp));
        // }
        // RCLCPP_INFO(get_logger(), "Scan time range: %f to %f", timeScanCur, timeScanEnd);
